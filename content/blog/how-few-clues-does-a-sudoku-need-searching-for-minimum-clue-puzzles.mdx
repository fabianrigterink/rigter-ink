...
This is the second post in a short series on Sudoku and mathematical optimization. The first post built a solver; here we put it to work on a harder question — how *few* clues a puzzle can have and still admit a single solution.

## From solving to questioning

In the [previous post](/blog/validating-and-solving-sudoku-puzzles-using-mathematical-optimization), we built a MIP-based Sudoku solver that can both solve a puzzle and verify that its solution is unique. Along the way, we encountered a 17-clue puzzle — the minimum possible for a standard $9 \times 9$ Sudoku.
...
![Convergence plot: best-so-far minimum clues vs. iteration for the 4×4 Sudoku search](https://res.cloudinary.com/dwp6mzleh/image/upload/q_auto/f_auto/v1779742224/01_minimum_clues_found_vs_iteration_4x4_znglj0.png "Convergence plot: best-so-far minimum clues vs. iteration for the 4×4 Sudoku search")
...
![Convergence plot: best-so-far minimum clues vs. iteration for the 9×9 Sudoku search](https://res.cloudinary.com/dwp6mzleh/image/upload/q_auto/f_auto/v1779742224/02_minimum_clues_found_vs_iteration_9x9_yjvudt.png "Convergence plot: best-so-far minimum clues vs. iteration for the 9×9 Sudoku search")
...
![Convergence plot: best-so-far minimum clues vs. iteration for the 16×16 Sudoku search](https://res.cloudinary.com/dwp6mzleh/image/upload/q_auto/f_auto/v1779742224/03_minimum_clues_found_vs_iteration_16x16_wmjr1w.png "Convergence plot: best-so-far minimum clues vs. iteration for the 16×16 Sudoku search")
...
*The full code — including the solver, the search procedure, and the convergence plots — is available as a [Colab notebook](https://colab.research.google.com/drive/1UJhPmrB0p9f5Oqf03obDC9s3vRm-8Qa5?usp=sharing).*
